...
In this chapter we explore the social technical impacts of the Euro 2.0 system on key stakeholders. We explore the research question:
...
\subsection{Research Methods} \label{ssec:4.2}

\subsubsection{Change Formula} \label{sssec:4.2:changeFormula}
We approach the research questions design and UX influenced interpretation of the \textit{Formula for Change} from the field of Organization Development. Originally written on a chalkboard by scientist David Gleicher, the formula stuck in the mind of organizational behavior expert Richard Beckhard, and first published in Sloan Management Review\cite{beckhard1975} and later in book by Beckhard and Harris.\cite{organizationalTransitions1977} In the review paper, Cady et al. state ``The formula describes the conditions, that when met, will move an individual, group, or whole system in a direction of their choosing.''\cite{changeFormula2014}

The improved version of the formula by Dannemiller\cite{dannemiller1992} is described that change will occur when the following conditions are satisfied:
...
If any one of $D$, $V$, or $F$ is zero, i.e. neglected or overlooked, then the resistance to change will not be overcome.

\subsubsection{Change Analysis} \label{sssec:4.2:changeAnalysis}

\todo[inline]{Diagram of your stuff...}
