...
Bitcoin has a major benefit low cost transactions. Currently the transactions are free, since miners are incentivised enough by Bitcoin rewards mining new blocks of transactions. When Bitcoin supply approaches its upper bound, already 99\% of Bitcoins will be mined by 2036, then transaction fees will take over as incentive for miners to maintain the network\todo{find reference for network transaction fees}.   

\subsection{Bitcoin and Trust}
Bitcoin gained an enormous amount of infamous attention with the Silk Road, the ``Ebay for Drugs''\cite{EbayForDrugs}. The website amassed over 1000 vendors, tens of millions of dollars of revenue, and triple digit yearly growth until it was shutdown by the FBI in 2013\cite{NotAnEbayForDrugs} after two and a half years operation. The buyers were protected in escrow Bitcoin transaction, only releasing the funds to the sellar if the buyer is completely satisfied. Bitcoin allowed payments amongst parties that can barely trust each other in real life. The lack of traceable identity in pseudo-anonymity and lack of regulation makes Bitcoin a prime choice of payments for drug dealers and money launderers \cite{Ali:2015:BPU:2990603.2990632}.

\textit{``Bitcoin’s strengths and weaknesses both derive from the same essential ideological and architectural design choices.''}\cite{Ali:2015:BPU:2990603.2990632}

\textit{``Anonymity and lack of regulation which is meant to free users from central authorities also empowers drug dealers and money launderers.''}\cite{Ali:2015:BPU:2990603.2990632}.
