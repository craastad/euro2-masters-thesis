...
\subsubsection{Identity Server} \label{sssec:3.5:idServer}
...
\subsubsection{Identity Database} \label{sssec:3.5:idDatabase}
...
\begin{itemize}
	...
	\item \textbf{escrow} - stores escrow transactions to an ID with columns: PID \textit{id\_code}, encrypted \textit{private\_key}, Ethereum \textit{address}, boolean \textit{cleared}, and \textit{clearing\_hash}
...
Second would be to reduce the scope of the security analysis altogether to just the Ethereum smart contracts. There is substantial research about smart contract vulnerabilities that could fuel a more focused and fruitful code level security analysis of the smart contracts involved with the Euro 2.0 system\cite{cryptoeprint:2016:1007}\cite{Atzei2017}\cite{cryptoeprint:2015:460}.
...
The improved version of the formula by Dannemiller\cite{dannemiller1992} states that change will occur when the following condition is satisfied as represented by the qualitative equation:
...
We use the \textit{Change Formula} in the context of Euro 2.0 to analyze the change of stakeholders using Euro 2.0 digital currency instead of Euro in the traditional monetary system. This is the background for analysis technique proposed in the following section.

\subsubsection{Change Analysis} \label{sssec:4.2:changeAnalysis}

\textit{Change Analysis} is a UX and User Design interpretation of the \textit{Change Formula}. It is a framework to analyze how a new system will affect key stakeholders and avenues of suggest directions of future design research. The concepts of \textit{Change Analysis} are depicted in \hyperfigureref{fig:changeAnalysis} and described below. Stakeholders refer to human or organizational actors interacting with the system.
...
\textbf{Enablers} are the \textit{Patterns}, \textit{Habits}, \textit{Behaviors}, and \textit{Motivation} in the current state that would indicate stakeholders are ready to adapt the proposed change in the future state of the world. In regards to the Change Formula, enablers are those factors supporting first steps to the change $F$.

\textbf{Inhibitors} are the factors in the current state that would discourage stakeholders from the proposed change to the future state of the world including \textit{Existing Policy Legislation}, \textit{Parties with Vested Interest}, \textit{Infrastructure and Technology}, and \textit{Personal Barriers to Change}. In regards to the Change Formula, inhibitors are those factors contributing to resistance of change $R$.

The outcome of the change analysis is an understanding of how the change will impact the stakeholders, a set of inhibitors and enablers for that change, and further areas of design research to validate or disprove the proposed hypotheses.
...
\begin{itemize}
	...
	\item They lose money to opportunity cost of unusable money in transit
	\item They lose money by spending
...
\begin{itemize}
	\item They keep other peoples' money safe
	...
	\item They verify and identify people for AML \& KYC reasons
...
\begin{itemize}
	...
	\item They want to keep regulators happy to stay in business
\end{itemize}
...
\begin{itemize}
	\item It loses money in tax evasion
	\item It loses money on defaulted loans
\end{itemize}
...
\begin{itemize}
	\item Transactions and balances are on a public ledger of Ethereum addresses
	...
	\item Reserve bank can convert back and forth between Euro and digital Euro
	...
	\item No money admin fees for holding assets in the system
...
\begin{itemize}
	\item Less fees when financial intermediary is removed from transactions
	\item Every transaction is automatically on record
	\item The good and bad side affects of transparency of money
	\item No money stuck in transit in pending transactions
	\item Simpler taxes
	\item Limited to transact with on record goods and services (no under the table cash transactions)
...
\begin{itemize}
	\item Easier to track money
	\item Can freeze and seize assets more easily
	\item Can collect taxes without a middleman or fees
	\item Enables realtime tax collection
	\item Eliminate tax evasion
	\item Accurate statistics of the state and amount of money in the entire monetary system
\end{itemize}
...
\begin{itemize}
	\item Users may not be ready to manage their own money.
	...
	\item Users will not adopt a new digital currency if the goods and services they purchase everyday do not accept the digital currency.
	\item Users don't understand the implications of a public ledger of transactions.
...
\begin{itemize}
	\item Merchants will save money accepting payment with low fees of digital currency over existing solutions.
	\item Merchants will save time with automation of accounting and taxes.
...
\begin{itemize}
	...
	\item Merchants intentionally accepting cash for off the record transactions have no incentive to use a transparent system.
	\item Installing new technology or systems to accept payments has overhead costs.
	\item Accounting and tax firms will resist automation of their businesses.
\end{itemize}
...
\begin{itemize}
	\item Research validating the current relationship of money for each of the stakeholders.
	...
	\item Research supporting or rejecting the hypotheses of the of the enablers and inhibitors to change presented in this chapter.
\end{itemize}
...
After explaining the features and implementation details of the Euro 2.0 system in \hyperchapterref{sec:3} and exploring the change in relationship with money of stakeholders \hyperchapterref{sec:4}, we are ready to analyze the security risks of Euro 2.0 system. This chapter answers the research question:
...
\begin{tabular}{ | l | p{12cm} | }
  ...
  \\ \hline
  \textbf{High} & Severe disruption, more than 60 minute delay for payment operations.
  \\ \hline
\end{tabular}
\end{center}
...
These are chosen as critical information assets to be representative of the majority of information assets enumerated in \hypertableref{tab:informationAssets}. The critical information assets are described in more detail in \hypertableref{tab:assetProfileUserMoney}, \hypertableref{tab:assetProfileEthereumAdminKeys}, and \hypertableref{tab:assetProfileUserIdentity}. Limiting the number of critical information assets reduces the number of risk profiles in \hypersectionref{ssec:5.3} and simplifies the complexity of the risk analysis in \hypersectionref{ssec:5.4}. \textit{Ethereum Contract Data} and \textit{User Keys} were also considered as critical information assets but are adequately represented by \textit{User Money}. \textit{User Transfer History} was also considered as a critical information asset but is related enough to \textit{User Identity} to be disregarded.
...
\begin{tabular}{ | l | l | l | p{10cm} |}
  ...
  \\ \hline
  Identity Server & \specialcell{Technical \\ Internal} & Identity & The Identity server currently handles escrow of funds to an ID and addresses and keys backup.
  \\ \hline
  ...
  \\ \hline
  User Client & \specialcell{Technical \\ External} & \specialcell{User \\ Convenience} & User device to interact with the Euro 2.0 system.
  \\ \hline
...
\begin{tabular}{ | l | l | l | p{10cm} |}
  ...
  \\ \hline
  Identity Server & \specialcell{Technical \\ Internal} & Identity & Server holding the approver key file needed to approve addresses for authorized IDs.
  \\ \hline
...
\begin{tabular}{ | p{5cm} | p{12cm} | }
  ...
  \textbf{(5) Security Requirements} &
  	\hskip-0.2cm
  	\begin{tabular}{p{2.5cm}|p{9.06cm}}
		\textbf{Confidentiality} & Highly confidential, only the Identity server, owning user, and other users that have been told that ID code.
		\\ \hline
		\textbf{Integrity} & The ID should be exactly as is for the Identity server or user approving an address.
		\\ \hline
...
\begin{tabular}{ | l | l | l | p{10cm} |}
  ...
  \textbf{Container} & \textbf{Type} & \textbf{Owners} & \textbf{Description}
  \\ \hline
  Identity Server & \specialcell{Technical \\ Internal} & Identity & Manages ID authentication and approval of Ethereum addresses for users.
  \\ \hline
  Identity Database & \specialcell{Technical \\ Internal} & Identity & Database storing ID to Ethereum address mapping, escrow, and keys backups.
  \\ \hline
  ...
  \\ \hline
  People & \specialcell{People \\ External} & Anyone & Knowledge of ID numbers can be found in many external documents and proccesses in Estonia.
  \\ \hline
\end{tabular}
}
\end{center}
...
\paragraph{(1) Identity or AML Database Hacked }
...
\begin{tabular}{ | l | p{14cm} |}
  ...
  (8) Consequences &
    \begin{itemize}
		\item Identity of all users' transactions is compromised and published for attacker gain.
...
\end{center}
\captionof{table}{Risk Profile: Identity or AML Database Hacked}
...
\end{tabular}
\end{center}
\captionof{table}{Severity: Identity or AML Database Hacked}
...
Another approach is for key storage to be completely delegated to a online third party, such as the popular exchange Coinbase. This creates a user experience with the same properties as an online bank\cite{Eskandari2015} suitable for users who want to avoid key management.
...
\paragraph{(III:1) Identity or AML Database Hacked}
...
\begin{itemize}
	...
	\item Move manual usage of critical admin keys to M of N signatures to reduce risks of misuse, loss, and bribery.
